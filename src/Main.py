...
def main():
    files_base = []
    for r, d, f in os.walk("D:\\Projects\\LeafRecognition\\data\\Original\\train\\Black-grass"):
        for file in f:
            if ".png" in file:
                files_base.append(os.path.join(r, file))

    files_comparative = []
    for r, d, f in os.walk("D:\\Projects\\LeafRecognition\\data\\Nonsegmented\\Black-grass"):
        for file in f:
            if ".png" in file:
                files_comparative.append(os.path.join(r, file))

    output_file = open("..\\data\\gabarito\\Blackgrass.csv", "w+")
